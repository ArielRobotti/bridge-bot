...
import { getTopPosters } from "../../services/messageTracker.js";

export async function handleTopPosters(interaction: ChatInputCommandInteraction): Promise<void> {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });

  const daysOption = interaction.options.getInteger("dias") ?? 7;
  const topUsers = getTopPosters(daysOption, 5);

  if (topUsers.length === 0) {
    await interaction.editReply(
      `No hay registro de actividad de mensajes (de ≥ 4 palabras) en los últimos ${daysOption} día(s).`
    );
    return;
  }
  ...
  topUsers.forEach((item, index) => {
    const position = `#${index + 1}`.padEnd(4);
    const name = `@${item.username}`.padEnd(20);
    lines.push(`${position} ${name} : ${item.messageCount} mensajes`);
  });
